    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {outlets.map((outlet) => (
          <button
            key={outlet.id}
            onClick={() => setSelectedId(selectedId === outlet.id ? null : outlet.id)}
          ...
          >
            ...
            {outlet.location && (
              <p className={`text-sm ${selectedId === outlet.id ? 'text-blue-100' : 'text-gray-500'}`}>
                {outlet.location}
              </p>
            )}
          </button>
        ...
        {outlets.length === 0 && (
          <p className="text-gray-400 col-span-3 text-center py-6">No outlets yet</p>
        )}
      </div>
